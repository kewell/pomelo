...
//Fix the error spectrum of CCK-40MHZ<--
		...
		if(pAd->ate.TxWI.PHYMODE==MODE_CCK&&pAd->ate.TxWI.BW==BW_40)
			{
					DBGPRINT(RT_DEBUG_ERROR, ("Set_ATE_TX_BW_Proc!!Warning!! CCK only supports 20MHZ!!Switch  to 20.\n"));
					pAd->ate.TxWI.BW = BW_20;
			}
#endif // RT30xx //
//Fix the error spectrum of CCK-40MHZ-->
...
//Fix the error spectrum of CCK-40MHZ<--
		...
		{
			RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R4, &BbpData);
			BbpData &= (~0x18);
			RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R4, BbpData);
			pAd->ate.TxWI.BW = BW_20;
...
//2008/07/10:KH add to support 3070 ATE<--
	if(IS_RT30xx(pAd))
	{
		DBGPRINT(RT_DEBUG_TRACE,("Warning!! RT30xx Don't Support\n"));
		return FALSE;
	...
	IN	PUCHAR			arg)
{
	...
	if(IS_RT30xx(pAd))
	{
		DBGPRINT(RT_DEBUG_TRACE,("SET_ATE_3070RF_Proc=%s\n",arg));
		this_char =arg;
		if ((value = strchr(this_char, ':')) != NULL)
			*value++ = 0;
		Reg= simple_strtol(this_char, 0, 16);
		RFValue= simple_strtol(value, 0, 16);
		DBGPRINT(RT_DEBUG_TRACE,("RF Reg[%d]=%d\n",Reg,RFValue));
...

{
				...
				DBGPRINT(RT_DEBUG_TRACE, ("wrq->u.data.length = %d\n", wrq->u.data.length));
		            	...
		            	if (copy_to_user(wrq->u.data.pointer, (PUCHAR)pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_TRACE, ("copy_to_user() fail in case RACFG_CMD_ATE_START\n"));
		                   return -EFAULT;
		            	}
				else
				{
                			DBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_ATE_START is done !\n"));
				}
...
)
{
				...
				if (pRaCfg->length == sizeof(pAdapter->ate.AtePid))
				{
					...
					DBGPRINT(RT_DEBUG_TRACE, ("wrq->u.data.length = %d\n", wrq->u.data.length));
			            	...
			            	if (copy_to_user(wrq->u.data.pointer, (PUCHAR)pRaCfg, wrq->u.data.length))
			            	{
			            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_ATE_STOP\n"));
			                   return -EFAULT;
			            	}
					...
					if (ret)
					{
						DBGPRINT(RT_DEBUG_ERROR, ("%s: unable to signal thread\n", pAdapter->net_dev->name));
					}
#endif // !UCOS //
				}
...
)
{
				...
				DBGPRINT(RT_DEBUG_TRACE, ("wrq->u.data.length = %d\n", wrq->u.data.length));
		            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_RF_WRITE_ALL\n"));
		                    return  -EFAULT;
		            	}
				else
				{
...
{
				UINT16	offset=0, value, tmp;
				UCHAR temp_buffer[200];

				//KH debug<--
				offset = ntohs(pRaCfg->status);
				//offset = pRaCfg->status;
				//KH debug-->
				...
				NdisZeroMemory(temp_buffer, 200);
				...
				//
				//
				//pRaCfg->magic_no=ntohs(pRaCfg->magic_no);
				//pRaCfg->command_type=ntohs(pRaCfg->command_type);
				//pRaCfg->command_id=ntohs(pRaCfg->command_id);
				//pRaCfg->length=ntohs(pRaCfg->length);
				//pRaCfg->sequence=ntohs(pRaCfg->sequence);
				//pRaCfg->status=ntohs(pRaCfg->status);
				//
				*/
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
									+ sizeof(pRaCfg->command_id) + sizeof(pRaCfg->length) 
									+ sizeof(pRaCfg->sequence) 
									//KH debug-->
									//+pRaCfg->length;
									+ntohs(pRaCfg->length);
									//KH debug<--
				DBGPRINT(RT_DEBUG_TRACE, ("sizeof(struct ate_racfghdr) = %d\n", sizeof(struct ate_racfghdr)));
				DBGPRINT(RT_DEBUG_TRACE, ("wrq->u.data.length = %d\n", wrq->u.data.length));
				

	            	if (copy_to_user(wrq->u.data.pointer, pRaCfg, wrq->u.data.length))
	            	{
	            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_E2PROM_READ16\n"));
	                    return -EFAULT;
	            	}
			else
			{		
...

{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_E2PROM_WRITE16\n"));
		                    return -EFAULT;
		            	}
				else
				{
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer, (PUCHAR) pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_E2PROM_READ_ALL\n"));
		                   return -EFAULT;
		            	}
				else
				{
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer, (PUCHAR) pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_E2PROM_WRITE_ALL\n"));
		                    return -EFAULT;
		            	}
				else
				{
                			DBGPRINT(RT_DEBUG_ERROR, ("RACFG_CMD_E2PROM_WRITE_ALL is done !\n"));
				}
				 return NDIS_STATUS_SUCCESS;
...
IN struct ate_racfghdr *pRaCfg
...

			{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_IO_READ\n"));
		                    return -EFAULT;
		            	}
				else
				{
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer, (PUCHAR) pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_IO_WRITE\n"));
		                   return -EFAULT;
		            	}
				else
				{
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer, (PUCHAR) pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_IO_READ_BULK\n"));
		                   return -EFAULT;
		            	}
				else
				{
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_BBP_READ8\n"));
		                     return -EFAULT;
		            	}
				else
				{
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_BBP_WRITE8\n"));
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer, (PUCHAR) pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_BBP_READ_ALL\n"));
		                    return -EFAULT;
		            	}
				else
				{
...
)
{
			...
			wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
	            ...
	            if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
	            {
	            	DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_ATE_E2PROM_READ_BULK\n"));
	                return -EFAULT;
	            }
			else
			{
               	DBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_ATE_E2PROM_READ_BULK is done !\n"));
			}
			return NDIS_STATUS_SUCCESS;
...
)
{
			...
			wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
	            ...
	            if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
	            {
	            	DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_ATE_E2PROM_WRITE_BULK\n"));
	                  return -EFAULT;
	            }
			else
			{
               	DBGPRINT(RT_DEBUG_ERROR, ("RACFG_CMD_ATE_E2PROM_WRITE_BULK is done !\n"));
			}
			return NDIS_STATUS_SUCCESS;
...
)
{
			...
			wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
	            ...
	            if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
	            {
	            	DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_ATE_IO_WRITE_BULK\n"));
	                   return -EFAULT;
	            }
			else
			{
               	DBGPRINT(RT_DEBUG_ERROR, ("RACFG_CMD_ATE_IO_WRITE_BULK is done !\n"));
			}
			return NDIS_STATUS_SUCCESS;
...
)
{
			...
			wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
	            ...
	            if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
	            {
	            	DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_ATE_BBP_READ_BULK\n"));
	                  return -EFAULT;
	            }
			else
			{
               	DBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_ATE_BBP_READ_BULK is done !\n"));
			}
				return NDIS_STATUS_SUCCESS;
...
)
{
			...
			wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
	            ...
	            if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
	            {
	            	DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_ATE_BBP_WRITE_BULK\n"));
	                   return -EFAULT;
	            }
			else
			{
               	DBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_ATE_BBP_WRITE_BULK is done !\n"));
			}
			return NDIS_STATUS_SUCCESS;
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_GET_NOISE_LEVEL\n"));
		                    return -EFAULT;
		            	}
				else
				{
                			DBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_GET_NOISE_LEVEL is done !\n"));
				}
				return NDIS_STATUS_SUCCESS;
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_GET_COUNTER\n"));
		                    return -EFAULT;
		            	}
				else
				{
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_CLEAR_COUNTER\n"));
		                    return -EFAULT;
		            	}
				else
				{
...
)
{
				...
				if (pAdapter->ate.bQATxStart == TRUE)
				{
					...
					wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
			            	...
			            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
			            	{
			            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() was failed in case RACFG_CMD_TX_START\n"));
			                    return -EFAULT;
			            	}
					else
					{
...
TX_START_ERROR:
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_TX_START\n"));
		                    return -EFAULT;
		            	}
				else
				{
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_GET_TX_STATUS\n"));
		                   return -EFAULT;
		            	}
				else
				{
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer, (PUCHAR) pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_RX_START\n"));
		                    return -EFAULT;
		            	}
				else
				{
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_RX_STOP\n"));
		                    return -EFAULT;
		            	}
				else
				{
...
)
{
				...
				DBGPRINT(RT_DEBUG_TRACE, ("wrq->u.data.length = %d\n", wrq->u.data.length));
		            	...
		            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_ATE_START_TX_CARRIER\n"));
		                    return -EFAULT;
		            	}
				else
				{
                			DBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_ATE_START_TX_CARRIER is done !\n"));
				}
					return NDIS_STATUS_SUCCESS;
...
)
{
				...
				DBGPRINT(RT_DEBUG_TRACE, ("wrq->u.data.length = %d\n", wrq->u.data.length));
		            	...
		            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_ATE_START_TX_CONT\n"));
		                   return -EFAULT;
		            	}
				else
				{
                			DBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_ATE_START_TX_CONT is done !\n"));
				}
					return NDIS_STATUS_SUCCESS;
...
)
{
				...
				DBGPRINT(RT_DEBUG_TRACE, ("wrq->u.data.length = %d\n", wrq->u.data.length));
		            	...
		            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_ATE_START_TX_FRAME\n"));
		                    return -EFAULT;
		            	}
				else
				{
                			DBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_ATE_START_TX_FRAME is done !\n"));
				}
					return NDIS_STATUS_SUCCESS;
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_ATE_SET_BW\n"));
		                   return -EFAULT;
		            	}
				else
				{
		                	DBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_ATE_SET_BW is done !\n"));
				}
					return NDIS_STATUS_SUCCESS;
...
)
{
            	...
            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
            	{
            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_ATE_SET_TX_POWER0\n"));
                    return -EFAULT;
            	}
		else
		{
                	DBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_ATE_SET_TX_POWER0 is done !\n"));
		}
		return NDIS_STATUS_SUCCESS;
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_ATE_SET_TX_POWER1\n"));
		                    return -EFAULT;
		            	}
				else
				{
                			DBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_ATE_SET_TX_POWER1 is done !\n"));
				}
					return NDIS_STATUS_SUCCESS;
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer, (PUCHAR) pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_ATE_SET_FREQ_OFFSET\n"));
		                   	return -EFAULT;
		            	}
				else
				{
                			DBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_ATE_SET_FREQ_OFFSET is done !\n"));
				}
					return NDIS_STATUS_SUCCESS;
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_ATE_GET_STATISTICS\n"));
		                    return -EFAULT;
		            	}
				else
				{
                			DBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_ATE_GET_STATISTICS is done !\n"));
				}
					return NDIS_STATUS_SUCCESS;
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_ATE_RESET_COUNTER\n"));
		                    return -EFAULT;
		            	}
				else
				{
                			DBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_ATE_RESET_COUNTER is done !\n"));
				}
					return NDIS_STATUS_SUCCESS;
...
)	
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_ATE_SEL_TX_ANTENNA\n"));
		                    return -EFAULT;
		            	}
				else
				{
                			DBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_ATE_SEL_TX_ANTENNA is done !\n"));
				}
					return NDIS_STATUS_SUCCESS;
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_ATE_SEL_RX_ANTENNA\n"));
		                    return -EFAULT;
		            	}
				else
				{
                			DBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_ATE_SEL_RX_ANTENNA is done !\n"));
				}
					return NDIS_STATUS_SUCCESS;
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer, (PUCHAR) pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_ATE_SET_PREAMBLE\n"));
		                    return -EFAULT;
		            	}
				else
				{
                			DBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_ATE_SET_PREAMBLE is done !\n"));
				}
					return NDIS_STATUS_SUCCESS;
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer, (PUCHAR) pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_ATE_SET_CHANNEL\n"));
		                    return -EFAULT;
		            	}
				else
				{
                			DBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_ATE_SET_CHANNEL is done !\n"));
				}
					return NDIS_STATUS_SUCCESS;
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_ATE_SET_ADDR1\n"));
		                    return -EFAULT;
		            	}
				else
				{
					DBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_ATE_SET_ADDR1 is done !\n (ADDR1 = %2X:%2X:%2X:%2X:%2X:%2X)\n", pAdapter->ate.Addr1[0],
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_ATE_SET_ADDR2\n"));
		                     return -EFAULT;
		            	}
				else
				{
					DBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_ATE_SET_ADDR2 is done !\n (ADDR2 = %2X:%2X:%2X:%2X:%2X:%2X)\n", pAdapter->ate.Addr2[0],
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer, (PUCHAR) pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_ATE_SET_ADDR3\n"));
		                    return -EFAULT;
		            	}
				else
				{
					DBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_ATE_SET_ADDR3 is done !\n (ADDR3 = %2X:%2X:%2X:%2X:%2X:%2X)\n", pAdapter->ate.Addr3[0],
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_ATE_SET_RATE\n"));
		                    return -EFAULT;
		            	}
				else
				{
                			DBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_ATE_SET_RATE is done !\n"));
				}
					return NDIS_STATUS_SUCCESS;
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer,  (PUCHAR)pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_ATE_SET_TX_FRAME_LEN\n"));
		                    return -EFAULT;
		            	}
				else
				{
                			DBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_ATE_SET_TX_FRAME_LEN is done !\n"));
				}
					return NDIS_STATUS_SUCCESS;
...
)
{
				...
				wrq->u.data.length = sizeof(pRaCfg->magic_no) + sizeof(pRaCfg->command_type)
		            	...
		            	if (copy_to_user(wrq->u.data.pointer, (PUCHAR) pRaCfg, wrq->u.data.length))
		            	{
		            		DBGPRINT(RT_DEBUG_ERROR, ("copy_to_user() fail in case RACFG_CMD_ATE_SET_TX_FRAME_COUNT\n"));
		                    return -EFAULT;
		            	}
				else
				{
                			DBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_ATE_SET_TX_FRAME_COUNT is done !\n"));
				}
				return NDIS_STATUS_SUCCESS;
