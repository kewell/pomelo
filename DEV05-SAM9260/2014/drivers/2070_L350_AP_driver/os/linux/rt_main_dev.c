...
/* added by guowenxue on 2009.09.04 */
#include <asm/arch/gpio.h>
#include <asm/arch-at91/hardware.h>
#include <asm/io.h>

#define BLINK_TIMEOUT           10
#define WIFI_LED                AT91_PIN_PB31 

struct timer_list               wifi_led_timer;
unsigned char                   ucFlag = 0x00;
/* add end */
...
/* add by guowenxue 2009.09.04 */
        del_timer(&wifi_led_timer);               // delete wifi led timer
        at91_set_gpio_value(WIFI_LED, 1);         // turn off LED
/*  add end */
	...
	if (pAd->UsbVendorReqBuf)
        ...
        if (pAd->Winbond_UsbVendorReqBuf)
                os_free_mem(pAd, pAd->Winbond_UsbVendorReqBuf);
#endif // WINBOND //
#endif // RT2870 //
...
#endif // INF_AMAZON_SE //
#ifdef WINBOND
        init_MUTEX(&(pAd->Winbond_UsbVendorReq_semaphore));
        os_alloc_mem(pAd, (PUCHAR)&pAd->Winbond_UsbVendorReqBuf, MAX_PARAM_BUFFER_SIZE - 1);
        if (pAd->Winbond_UsbVendorReqBuf == NULL)
        {
                DBGPRINT(RT_DEBUG_ERROR, ("Allocate vendor request temp buffer failed!\n"));
                goto winbond_err0;
        }
...
/*=======================================================================================================
 * If recv or send data in  rt_linux.c and rt_main_dev.c, the wifi_led_timer.data will be set as 0,
 * So now, the first time when "data==0", LED will be turn off, and the secend time will be turn on to 
 * let it blink.
 *
 * The count value used to avoid both ucFlag and data still be 0x00 when data transfer very slow.
 *=====================================================================================================*/
void wifi_led_off(unsigned long data)
{
    static unsigned char          count = 0;
    ucFlag = (~ucFlag) & 0x01;    // ucFlag swap between 1 and 0.

    if(0==data && 0x01==ucFlag) // Turn LED off
    {
        at91_set_gpio_value(WIFI_LED, 1);
        count = 0;
    }
    else  // Turn LED on 
    {
        if(0x01==ucFlag) //Has data transfer by wifi
        {
           if(2 != count) // Has data transfer but doesn't turn off the led 
             count ++;
           else // Data hasn't transfer for twice but should turn off.
           {
             ucFlag = 0x00;
           }
        }

        at91_set_gpio_value(WIFI_LED, 0);
    }
    wifi_led_timer.data = 1; // Default turn LED on
    mod_timer(&wifi_led_timer, jiffies+BLINK_TIMEOUT);
...
/*  add by guowenxue 2009.09.04 */
        at91_set_gpio_output(WIFI_LED, 0); // Turn LED on when do "ifconfig ra0 up"

        printk("Initial and add wifi led timer in rt28xx_open().\n");
        init_timer(&wifi_led_timer);
        wifi_led_timer.expires = jiffies + BLINK_TIMEOUT;
        wifi_led_timer.function = wifi_led_off;
        wifi_led_timer.data = 1;
        add_timer(&wifi_led_timer);
/* add end */
...
int rt28xx_packet_xmit(struct sk_buff *skb)
{
	...
	PNDIS_PACKET pPacket = (PNDIS_PACKET) skb;
        ...
        wifi_led_timer.data = 0;  // add by guowenxue 2009.09.04, send data and need set WiFi LED blink.
