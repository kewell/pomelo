...
//#endif
					...
					{
						// WpaMixPairCipher
						if(RTMPGetKeyParameter("WpaMixPairCipher", tmpbuf, 256, buffer))
						{
							/*
					...
					IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
					{
						...
						//WscConfMethods
						if(RTMPGetKeyParameter("WscConfMethods", tmpbuf, 10, buffer))
						{
							pAd->ApCfg.MBSSID[MAIN_MBSSID].WscControl.WscConfigMethods = (USHORT)simple_strtol(tmpbuf, 0, 16);

							DBGPRINT(RT_DEBUG_TRACE, ("WscConfigMethods=%x\n", pAd->ApCfg.MBSSID[MAIN_MBSSID].WscControl.WscConfigMethods));
						}
#if 0
                    //WscPinCode
					...
					if (RTMPGetKeyParameter("EntryLifeCheck", tmpbuf, 256, buffer))
					{
						ULONG LifeCheckCnt = simple_strtol(tmpbuf, 0, 10);
