...
#ifdef RT2870
#ifdef CONFIG_AP_SUPPORT
	{PBF_CFG, 				0xf4000E}, 		// Enable Queue 1 and Queue 2
#endif // CONFIG_AP_SUPPORT //
	...
	IN	PUCHAR			mac_addr)
{
	...
	if (IS_RT30xx(pAd))
	{
		RT28xx_EEPROM_READ16(pAd, EEPROM_TXMIXER_GAIN, value);
		pAd->TxMixerGain = 0;
		value &= 0x00ff;
		if (value != 0xff)
		{
			value &= 0x07;
			pAd->TxMixerGain = (UCHAR)value;
		}
	}
#endif // RT30xx //
...
// add by johnli, RF power sequence setup
	...
	if (IS_RT3090(pAd))	// RT309x, RT3071/72
	{
		// enable DC filter
		if ((pAd->MACVersion & 0xffff) >= 0x0211)
		{
...
#define NICLF_DEFAULT_USE()	\
	...
	PUCHAR			src;
	struct file		*srcf;
	INT 			orgfsuid, orgfsgid, retval, i;
   	mm_segment_t	orgfs;
...
//	EDCA_PARM DefaultEdcaParm;
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		...
		for(j = BSS0; j < pAd->ApCfg.BssidNum; j++)
		{
			...
			pAd->ApCfg.MBSSID[j].WscControl.WscConfigMethods= 0x0084;
