	...
	IN	PMAC_TABLE_ENTRY	pEntry)
{
	...
	APToWirelessSta(pAdapter, pEntry, Header802_3, sizeof(Header802_3), (PUCHAR)pOutBuffer, FrameLen, TRUE);
	...
	IN  PUCHAR          pBssid)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAdapter)
	{
		...
		if (pEntry)
			APToWirelessSta(pAdapter, pEntry, Header802_3, sizeof(Header802_3), (PUCHAR)&Packet, 4, TRUE);
	}
#endif // APCLI_SUPPORT //
#endif // CONFIG_AP_SUPPORT //
	...
	IN	PRTMP_ADAPTER		pAdapter)
{
	...
	MAKE_802_3_HEADER(Header802_3, 
					  &pAdapter->ApCfg.MBSSID[apidx].WscControl.EntryAddr[0], 
	...
	pEntry = MacTableLookup(pAdapter, &pAdapter->ApCfg.MBSSID[apidx].WscControl.EntryAddr[0]);
	// Copy frame to Tx ring
	APToWirelessSta(pAdapter, pEntry, Header802_3, sizeof(Header802_3), (PUCHAR)pOutBuffer, FrameLen, TRUE);
	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	if (flg_pressed)
	{
		...
		if (pAd->CommonCfg.WscHdrPshBtnCheckCount == WSC_HDR_BTN_CONT_TIMES)
		{
			...
			{
				pObj->ioctl_if = 0;
				Set_AP_WscMode_Proc(pAd, "2"); /* 2: PBC */
				Set_AP_WscGetConf_Proc(pAd, "1"); /* 1: Trigger */
			}
#endif // CONFIG_AP_SUPPORT //
