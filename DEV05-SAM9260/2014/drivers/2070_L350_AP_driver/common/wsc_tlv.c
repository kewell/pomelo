	...
	OUT	VOID *pbuf)
{
	...
	//10. Config Method
	pReg->EnrolleeInfo.ConfigMethods = cpu2be16(pWscControl->WscConfigMethods);
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAdapter)
		pReg->EnrolleeInfo.ScState = pWscControl->WscConfStatus;
	...
	OUT	VOID *pbuf)
{
	...
	SHA256(&pReg->SecretKey[0], 192, &DHKey[0]);
	...
	OUT	PWSC_REG_DATA pReg)
{
	...
	while (Length > 4)
	{
		...
		switch (WscType)
		{
			...
			case WSC_ID_CONFIG_METHODS:
				pReg->RegistrarInfo.ConfigMethods = be2cpu16(*((PUSHORT) pData));				
				break;
