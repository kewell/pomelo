...
//			pAd->ApCfg.MBSSID[apidx].CapabilityInfo |= 0x0200;
	...
	
	{
		...
		{	// request by Gary 20070208
			...
			if (IS_RT30xx(pAd))
			{
				byteValue = 0x1C + 2*GET_LNA_GAIN(pAd);
				RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R66, byteValue);
			}
			else
#endif // RT30xx //
	...
	for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++)
	{
		...
		if (IS_WPA_CAPABILITY(pAd->ApCfg.MBSSID[apidx].AuthMode))
		{   
...
//		if ((PID & 0x3f) < WDS_PAIRWISE_KEY_OFFSET) // send to client
		{
            ...
            MiniportMMRequest(pAd, MapUserPriorityToAccessCategory[7], (PCHAR)pNullFr, Length);
		}
//#ifdef  WDS
#if 0
