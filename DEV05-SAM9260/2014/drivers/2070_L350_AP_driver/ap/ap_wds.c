	...
	for(index = 0; index < MAX_WDS_ENTRY; index++)
	{
		...
		{// find available 
			for (i = 0; i < 32; i++)
			{
				...
				device = dev_get_by_name(dev_net(net_dev), slot_name);
#else
	...
	do
	{
				...
				&& (pAd->WdsTab.WdsEntry[WdsIdex].WdsKey.KeyLen > 0))
		{
			...
			if (!VAILD_KEY_INDEX(DefaultKeyId))
				break;
#ifdef RT2870
			/* carella modify: WDS set PairwiseKey and WcidAttribute in Cmd Thread. */
			{
				RT_ADD_PAIRWISE_KEY_ENTRY KeyInfo;
				COPY_MAC_ADDR(KeyInfo.MacAddr,pEntry->Addr);
				KeyInfo.MacTabMatchWCID = pEntry->Aid;
				NdisMoveMemory(&KeyInfo.CipherKey,  &pAd->WdsTab.WdsEntry[pEntry->MatchWDSTabIdx].WdsKey,sizeof(CIPHER_KEY));
				RTUSBEnqueueInternalCmd(pAd, RT_CMD_SET_KEY_TABLE, &KeyInfo, sizeof(RT_ADD_PAIRWISE_KEY_ENTRY));
				RTUSBEnqueueInternalCmd(pAd, RT_CMD_SET_RX_WCID_TABLE, pEntry, sizeof(MAC_TABLE_ENTRY));
			}
#endif // RT2870 //
