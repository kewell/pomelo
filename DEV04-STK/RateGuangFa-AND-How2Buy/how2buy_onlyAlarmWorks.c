...
void analysia_each_stk (char *pcData)
{
    ...
    if (1 == canSell)
    {
        ...
        else if (1 && ((highVal - g_lastVal) / g_lastVal * 100 > g_fSellRate))  //compare everyDay Rate
        {
            sellEnd = 1;
        ...
        if (1 == sellEnd)
        {
	        ...
	        canSell = 0;
	        sellEnd = 0;
	        printf("%-10s S=%-6.2f OFF=%-6.2f  H=%-6.2f %-6.2f L=%-6.2f %-6.2f", date, g_sellVal, offVal, highRate, highVal,lowRate,lowVal);
    ...
    else if (1 == canBuy)
    {
        if ((g_lastVal - lowVal) / g_lastVal * 100 > g_fBuyRate)
        {
	        ...
	        printf("%-10s B=%-6.2f OFF=%-6.2f  H=%-6.2f %-6.2f L=%-6.2f %-6.2f\n", date, g_buyVal, offVal, highRate, highVal,lowRate,lowVal);
        }
    }
...
int main (int argc, char **argv)
{
    ...
    if (NULL != out)
    {
        printf("-----BuyRate=%.2f sellRate=%.2f lossAlarmRate=%.2f winAlarmRate=%.2f-----\n", 
