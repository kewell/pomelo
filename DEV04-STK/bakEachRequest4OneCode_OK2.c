...
char* analysia_each_stk (char *pcData, unsigned char isFirst, unsigned char index, char *pcRetStr)
{
...
*/
    ...
    float curPrice = val[2], yesterdayClosePrice = val[1];
    float fTmpRate = (curPrice - yesterdayClosePrice) * 100 / yesterdayClosePrice;
...
*/
        for (i = 27; i >= 19; i -= 2)
        {
            if (1 == bigReqFlag[i]) sprintf(pcSendTmp, "\033[1;33m% -4.0f\033[0;39m", val[i]/100);
            else sprintf(pcSendTmp, " %-4.0f", val[i]/100);
        ...
        if (1 == useLargeFlag)
        {
            ...
            {
                sprintf(pcSendTmp, "\033[1;31m%-4.0f\033[0;39m", curTradeSum);
            }
            else if (curPrice > g_afLastPrice[index])
            {
                sprintf(pcSendTmp, "\033[1;34m%-4.0f\033[0;39m", curTradeSum);
            }
            else
            {
                sprintf(pcSendTmp, "\033[1;33m%-4.0f\033[0;39m", curTradeSum);
            }
        }
        else
        {
            sprintf(pcSendTmp, "%-4.0f", curTradeSum);
        }
        strcat(pcSend, pcSendTmp);
        memset(pcSendTmp, 0, MAXIMAL_BUF_SIZE);
    ...
    write(ptsFd, pcSend, strlen(pcSend));    
    int fileBakFd = open(g_acBakFileNa, O_RDWR|O_APPEND);//O_APPEND);
    if (0 < fileBakFd)
    {
        write(fileBakFd, pcSend, strlen(pcSend));
    }
    close(ptsFd);
    close(fileBakFd);
#endif
            ...
            while (getline(&eachData, &len, out) != -1)
            {
                if (g_uc1stCodeFromData || strlen(eachData) > EACH_REALTIME_DATA_MINIMAL_LEN)
                {
                    if (NULL != eachData)
                    {
                        ...
                        analysia_each_stk(eachData, isFirst, index, outPutMsgTmp);
                        g_uc1stCodeFromData = 0;
