...
int iRunCnt = 1;
#define FILE_OUTP           "/dev/.kts"
#define USEFUL_ID           0 
#define RUN_ONCE            "1"
#define RUN_NO_STOP         "0"
#define ASCII_0_VAL         0x30
#define SHOW_GAP_CNTS       142

int g_ucArgDozeSec = 15;
unsigned char g_ucArgGapCnt = 20;
...
int checkTradeTime()
{
    // return 1 ---- is not trade time
    // return 0 ---- is     trade time 09:28--11:32   12:58--15:02
    struct timeb stTime;
    struct tm *pstTime;
    int iHour = 0, iMin = 0, iSec = 0;
    ftime(&stTime);
    pstTime = localtime(&(stTime.time));
    iHour = pstTime->tm_hour;
    iMin = pstTime->tm_min;
    iSec = pstTime->tm_sec;

    if (0 == tradeTimeCheck(iHour, iMin, iSec))
    {
        return 0;
    }
    else
    ...
    if(g_ucRunNoStop) 
    {
        ...
        if(0 < pts)
        {
            ...
            if (g_ucIsSZA)
            {
                if (fTmpRate > 0)
                {
                    sprintf(pcSend, "\n%02d%02d\033[0;33m%02d\033[0;39m", min,sec, (int)(fTmpRate*10));
                }
                else
                {
                    sprintf(pcSend, "\n%02d%02d\033[0;37m%02d\033[0;39m", min,sec, abs((int)(fTmpRate*10)));
                }
            }
            else
            {
                ...
                else if (1 == g_showTmpRateFlg)
                {
                    if (0 == strcmp(pcStockName, "0001") || 0 == strcmp(pcStockName, "9001") || 0 == strcmp(pcStockName, "9005") || 0 == strcmp(pcStockName, "9006"))
                    {
                        if (fTmpRate > 0)
                        {
                            sprintf(pcSend, "\033[0;33m%02d\033[0;39m", (int)(fTmpRate*10));  
                        }
                        else
                        {
                            sprintf(pcSend, "\033[0;37m%02d\033[0;39m", abs((int)(fTmpRate*10)));
                        }
                    }
                    else if (fTmpRate > g_afTmpRate[index])
                    {
                        sprintf(pcSend, " \033[0;33m%s%.1f\033[0;39m", ((fTmpRate >= 0.0) ? "+" : ""), fTmpRate);
                    }
                    else if (fTmpRate < g_afTmpRate[index])
                    {
                        sprintf(pcSend, " \033[0;37m%s%.1f\033[0;39m", ((fTmpRate >= 0.0) ? "+" : ""), fTmpRate);
                    }
                    else
                    {
...
int main (int argc, char **argv)
{
    ...
    {
        iRunCnt--;
        if (1 == g_ucRunNoStop && 1 == checkTradeTime())
        {
            continue;
        }
