...
/*************************************************
  宏定义
*************************************************/
#define DRIVER_NAME "atmel_tc"
#define MAJOR_DEVICE	105 
#define MINOR_MAX 6
...
#define MCK 99000000  // system bus clock
#define eTCLK0 32768	//external 0 clock freq
#define eTCLK1 32768
#define eTCLK2 32768
...
struct atmel_tc {	
	char  name[20];
	int		id;					//定时器功能，心跳，PWM，频率测量
	u32		pulse;				//协同开入驱动开关
	u32		ready;
	unsigned int	block;
	unsigned int	channle ;

	void __iomem	*blkregs;	//定时器硬件资源
	void __iomem	*regs;
	struct clk		*clk;
	...
	int		clock_source;		//定时器计数时钟源
	u32		clock_f;			//相应时钟源对应的时钟频率
