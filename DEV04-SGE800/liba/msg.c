...
static struct {
    ...
    u32 head;
    u32 tail;
    message_t message[CFG_MSG_REALSIZE];
...
//读出消息并排列好剩余的消息
    ...
    if (index == head) {
        ...
        memmove(&msg_info[id].message[head] + 1, &msg_info[id].message[head], (index - head) * sizeof(message_t));
    	msg_info[id].head = (head + 1) % CFG_MSG_REALSIZE;
    }
    else {
        memmove(&msg_info[id].message[index], &msg_info[id].message[index] + 1, (tail - index - 1) * sizeof(message_t));
        if(0 == tail) {
