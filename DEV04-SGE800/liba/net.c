...
*	说明:	无
******************************************************************************/
int net_client_init(u16 timeout)
{
	int ret;
	struct timeval tout;
	//创建socket
	if ((client_net_info.sockfd = socket(AF_INET, SOCK_STREAM, 0)) == -1){
		ret = -ERR_SYS;
		goto error;
	}
	//设置网络阻塞接收、发送超时时间
	...
	tout.tv_usec = 0;
	//设置发送超时
	ret = setsockopt(client_net_info.sockfd, SOL_SOCKET, SO_SNDTIMEO, &tout, sizeof(tout));
	if (ret)
	{
		ret = -ERR_SYS;
		goto error;
	...
	//设置接收超时
	ret = setsockopt(client_net_info.sockfd, SOL_SOCKET, SO_RCVTIMEO, &tout, sizeof(tout));
	if (ret)
	{
		ret = -ERR_SYS;
		goto error;
...
*	说明:	无
******************************************************************************/
int net_client_connect(char *ip,u16 port)
{
	int ret = 0;
	...
	//发起对服务器端的连接
	if (connect(client_net_info.sockfd, (struct sockaddr *)&serv_addr, sizeof(struct sockaddr)) == -1){
...
*	说明:	无
******************************************************************************/
int net_client_receive(u8 *buf, u16 max_length, u16 *length, u8 flag)
{
	int recvbytes,ret;
	int errnum;
	
	if (flag == NET_NONBLOCK){
		recvbytes = recv(client_net_info.sockfd, buf, max_length, MSG_DONTWAIT);    //非阻塞接收
	}else if (flag == NET_BLOCK){
 		recvbytes = recv(client_net_info.sockfd, buf, max_length, 0);               //阻塞接收
	}else{
		ret = -ERR_INVAL;
		goto error;
	}
...
*	说明:	无
******************************************************************************/
int net_client_send(u8 *buf,u16 length)
{
	int ret;
	int errnum;
	if (length == 0){
	...
	ret = send(client_net_info.sockfd, buf, length, 0);
	if (ret == -1){
		errnum = errno;
...
*	说明:	无
******************************************************************************/
int net_client_disconnect()
{
	int ref,ret;
	int errnum;
	ref = close(client_net_info.sockfd);
	if (ref == 0){
...
int net_server_connect(u8 *id)
{
	...
	if((server_net_info.client_fd[ud] = accept(server_net_info.sockfd, \
