...
int main()
{
	...
	//测试用例3
	ret = dbs_sync(0);
	assert(ret == -ERR_NOFILE, "dbs_sync Use Case 3 error");
	...
	//测试用例1
	ret = dbs_sync(0);
	assert(ret == 0, "dbs_sync Use Case 1 error");

	//测试用例2
	ret = dbs_sync(16);
	assert(ret == -ERR_INVAL, "dbs_sync Use Case 2 error");
