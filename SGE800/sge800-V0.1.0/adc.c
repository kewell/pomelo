...
static int 	adc_fd, adi_fd;			//外部和内部ad文件描述符
...
int adc_init(void)
{
	...
	memset(adc_chn,0xff,CFG_ADC_NUM);	//初始化adc通道值为无效
	//设置adc通道
#ifdef CFG_ADC_0
	adc_chn[0] = CFG_ADC_0 ;
#endif
	...
	if(adc_chn[0] != 0xff || adc_chn[1] != 0xff || adc_chn[2] != 0xff || adc_chn[3] != 0xff  ){
		adc_fd = open("/dev/tlv1504", O_RDONLY);	//打开外部ADC驱动
		if (adc_fd < 0){
	...
	if(adc_chn[4] != 0xff || adc_chn[5] != 0xff || adc_chn[6] != 0xff || adc_chn[7] != 0xff  ){
		adi_fd = open("/dev/atmel_adc", O_RDONLY);	//打开外部ADC驱动
		if (adi_fd < 0){
			ret = -ERR_NOFILE;		//没有此路径
			goto err;
		}
