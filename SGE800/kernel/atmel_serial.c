...
static void atmel_set_termios(struct uart_port *port, struct ktermios * termios, struct ktermios * old)
{
	...
	//roy add
	if(0xf0 == port->unused1){	//利用变量unused1传递485模式参数	
		//设置各io口rts功能
		if(AT91SAM9260_ID_US0 == (port)->irq)){
			printk("set termios: config 0 485 mode\n");
			//at91_set_A_periph(AT91_PIN_PB26, 0);  /* RTS0 */
		}
		if(AT91SAM9260_ID_US1 == (port)->irq)){
			printk("set termios: config 1 485 mode\n");
			//at91_set_A_periph(AT91_PIN_PB28, 0);  /* RTS1 */
		}
		if(AT91SAM9260_ID_US2 == (port)->irq){
			printk("set termios: config 2 485 mode\n");
			at91_set_A_periph(AT91_PIN_PA4, 0);  /* RTS2 */			
		}
		if(AT91SAM9260_ID_US3 == (port)->irq){
			printk("set termios: config 3 485 mode\n");
			at91_set_B_periph(AT91_PIN_PC8, 0);  /* RTS3 */
		}
		//设置模式寄存器
		mode &= ~ATMEL_US_USMODE;
		mode |= ATMEL_US_USMODE_RS485;
		port->unused1 = 0;	//系统不会清0，下次打开会保存此值。
	}
	//roy add end
...
//	static int	atmel_serial_ioctl(struct uart_port *port, unsigned int cmd, unsigned long arg)
//	{
//		if(cmd == 0x545f){
//			printk("atmel_serial_ioctl \n");
//			port->unused1 = 0xf0;
//		}
//		return 0;
//	}
