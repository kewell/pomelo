...
typedef struct DIS_FMT_T
{   
    unsigned char ucX;            /* Start pixel at COL */
    unsigned char ucY;            /* Start pixel at ROW */
    unsigned char ucWidth;        /* Font Width as pixel */
    unsigned char ucHeight;       /* Font Height as pixel */
    unsigned char ucGapX;         /* Blanks between each COL as pixel */
    unsigned char ucGapY;         /* Blanks between each ROW as pixel */
    unsigned char ucInvs;         /* Inverse flags, default is 0 means nerver inverse */
    unsigned char ucAlign;        /* If the font arrays widther than scrren, should reuse ucX/ucY? */
    unsigned char ucUnuse;
...
void dis_normal (U8 xMirror, U8 yMirror, U8 width, U8 height, U8 *p)
{
    ...
    each_row_byte = width / 8 + ((width % 8) ? 1 : 0);
...
static void display_string(DIS_FMT *fmt, U8 *font, int len)
{
    int row = 0, pos = 0, col = 0;
    int xMirror, yMirror;
    
    unsigned int uiEachFontSize = fmt->ucHeight * (fmt->ucWidth / 8 + ((fmt->ucWidth % 8) ? 1 : 0));
    unsigned char ucAlignFlag = 1;
    ...
    while (pos < len)
    {
         ...
         */
#if 1
        xMirror = fmt->ucX * ucAlignFlag + col * (fmt->ucGapX + fmt->ucWidth);
        col++;
        
        if ((xMirror + fmt->ucWidth) > 160)
        {
            if (fmt->ucAlign)
            {
                ucAlignFlag = 0;
        ...
        yMirror  = fmt->ucY + row * (fmt->ucGapY + fmt->ucHeight);

        if ((yMirror + fmt->ucHeight) > 160)
        {
            break;
        }

        dis_reverse(xMirror, yMirror, fmt->ucWidth, fmt->ucHeight, font);        
        //write_cmd(MAPPING_CTL | 0x04);dis_inverse(xMirror, yMirror, fmt->width, fmt->height, font);
#else
        yMirror  = fmt->ucY * ucAlignFlag + row * (fmt->ucGapX + fmt->ucWidth);
        row++;

        if ((yMirror + fmt->ucWidth) > 160)
        {
            if (fmt->ucAlign)
            {
                ucAlignFlag = 0;
        ...
        xMirror = (fmt->ucX + (col * (fmt->ucGapY + fmt->ucHeight)));
        if ((xMirror + fmt->ucHeight) > 160)
        {
            break;
        }
        
        dis_normal(xMirror, yMirror, fmt->ucWidth, fmt->ucHeight, font);
#endif
...
void simple_test (void)
{
    ...
    fmt = kmalloc(sizeof(DIS_FMT), GFP_KERNEL);
...
static ssize_t lcd_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
    ...
    if ((count < sizeof(*fmt)) || (count > 0x800)) 
        return -EFAULT;
    ...
    font = kbuf; 
    font += sizeof(*fmt);
    font_len = count - sizeof(*fmt);
    
    printk("invs=%d, unused=%d, len=%lu\n", fmt->ucInvs, fmt->ucUnuse, font_len);
    //printk("x=%d, y=%d, gapX=%d, gapY=%d, H=%d, W=%d\n", fmt->x, fmt->y, fmt->gapX, fmt->gapY, fmt->height, fmt->width);
    display_string(fmt, fmt->pucData, font_len);
...
{
	//int flag;
    //if (copy_from_user(&DIS_FMT, (DIS_FMT *)arg, sizeof(DIS_FMT)))
	return -EFAULT;
